...
def _merge_data(appId,ids, group_by, ext):
	...
	eStatAPI.path['tmp_merge'] = eStatAPI.path['tmp']+'.'.join([eStatAPI._['appId'], ''.join([l for l in random.choice(eStatAPI.random_str)]), 'csv'])
	data.to_csv(eStatAPI.path['tmp_merge'],quoting=csv.QUOTE_NONNUMERIC,index=None)
	tmp_csv = eStatAPI.cmd_line(eStatAPI.build_cmd(['cat',eStatAPI.path['tmp_merge']]))
