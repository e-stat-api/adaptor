...
class e_Stat_API_Adaptor:
	...
	def load_all_ids(self):
		...
		self.cmd_line(self.build_cmd([
			 'curl'
			,'-o'
			,self.path['statid-json']
			,'"'+load_uri+'"'
		]))
	# ダウンロードした統計表からインデックスファイルを作成する
	def build_statid_index(self):
		jd = self.load_json(self.path['statid-json'])['GET_STATS_LIST']['DATALIST_INF']['TABLE_INF']
		rows = '\n'.join([
			'-'.join([	
				 j['@id']
				,j['STAT_NAME']['$']
	...
	def load_stat_center_index(self):
		self.cmd_line(self.build_cmd([
			 'curl'
			,'-o'
			,self.path['dictionary-stat-center']
	...
	def build_detailed_index(self):
		...
		for i,j in enumerate(jd):
			...
			try:
				STATISTICS_NAME = self.create_n_gram_str(j['STATISTICS_NAME'], self.gram)
			except:
				STATISTICS_NAME = ''
			try:
	...
	def search_id( self, q, _index, _header='index'):
		if q == 'index':
			rows = [[c for c in f.split('-') if '.dic' not in c] for f in self.cmd_line(self.build_cmd(['cat',_index])).split('\n')]
		else:
			output = self.cmd_line('cat '+_index+' | '+'grep -n \"'+q+'\"').split('\n')
			rows = [[c if i > 0 else c.split(':')[-1] for i,c in enumerate(f.split('-')) if '.dic' not in c] for f in output]
	...
	def get_all_data(self, statsDataId, next_key):
		...
		try:
			if os.path.exists(self.cache['tmp']) == False:
				...
				self.cmd_line(self.build_cmd(['curl','-o',self.cache['tmp'],'"'+apiURI+'"'])).replace('\n','')
 			RESULT_INF = self.load_json(self.cache['tmp'])['GET_STATS_DATA']['STATISTICAL_DATA']['RESULT_INF']
			NEXT_KEY = '-1' if 'NEXT_KEY' not in RESULT_INF.keys() else RESULT_INF['NEXT_KEY']
			return str(NEXT_KEY)
		except:
			# 下記のエラー処理は考える
			filepath = self.path['tmp']+'.'.join([self._['appId'],statsDataId,'*','json'])
			try:
				downloaded_files = self.cmd_line(self.build_cmd(['ls',filepath]))
				if downloaded_files != '':
		...
		try:
			...
			ix = [
				...
				for f in self.cmd_line(
					self.build_cmd(['ls',self.path['tmp']+'.'.join([self._['appId'],statsDataId,'*','json'])])
				).split('\n') 
				if f != ''
			]
			print ix
	...
	def merge_data(self,statsDataId,group_by,aggregate):
		...
		if group_by != 'all':
			...
			# mean
				...
				data = data
		if group_by != 'all':
			data = data.loc[:,[c for c in data.columns if '$' in c or group_by in c]] if aggregate == '' else data.loc[:,[c for c in data.columns if '$' in c]]
		...
		if output_type == 'csv':
			...
			for i in range(1,len(tmp_data)):
				...
				for j in range(0, len(tmp_data[i])):
					row_data[tmp_data[0][j]] = get_tmp_data(tmp_data[0][j], tmp_data[i][j])
			...
			for i in range(0,len(tmp_data[0])):
				print tmp_data[0][i]
				data[tmp_data[0][i]] = [ get_tmp_data(tmp_data[0][i], tmp_data[j][i]) for j in range(1, len(tmp_data))]
			return json.dumps(data)
		else:
			return self.error(self.msg['check-extension'])
	def mimetype(self, ext):
		mt = 'text/plain' if ext ==  'csv' else 'application/json'
		mt = 'application/octet-stream' if request.args.get('dl') == 'true' else mt
		return mt
